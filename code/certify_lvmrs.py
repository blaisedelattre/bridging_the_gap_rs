...
def main(args):
    ...
    dataset = datasets.CIFAR10(
        CIFAR10_DATA_DIR, train=False, download=True, transform=transforms.ToTensor()
    )
    ...
    for i in range(len(dataset)):
        ...
        prediction, radius = smoothed_classifier.certify_ours(
            x, args.N0, args.N, args.alpha, args.batch_size
        )
    ...
    print(
        "sigma %.2f accuracy of smoothed classifier %.4f "
        % (args.sigma, correct / float(total_num))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--N", type=int, default=100000, help="number of samples to use"
    )
    parser.add_argument("--batch_size", type=int, default=1000, help="batch size")
    parser.add_argument(
        "--alpha", type=float, default=0.001, help="failure probability"
    )
